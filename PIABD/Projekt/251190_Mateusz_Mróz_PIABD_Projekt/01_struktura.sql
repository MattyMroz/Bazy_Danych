...
CREATE TABLE crunchbase.Company (
    ...
    total_money_raised DECIMAL(18, 2) NULL,
...
CREATE TABLE crunchbase.Investment (
    ...
    CONSTRAINT CK_Investment_ExclusiveInvestor CHECK (
        (CASE WHEN person_id IS NOT NULL THEN 1 ELSE 0 END +
         CASE WHEN financial_org_id IS NOT NULL THEN 1 ELSE 0 END +
         CASE WHEN investing_company_id IS NOT NULL THEN 1 ELSE 0 END) = 1
    )
);
GO
