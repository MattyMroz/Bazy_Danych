...
\begin{itemize}
    ...
    \item \textbf{Zarządzanie danymi nauczycieli} -- ewidencja kadry pedagogicznej, każdy nauczyciel uczy jednego przedmiotu (referencja REF do przedmiotu)
    ...
    \item \textbf{Kontrola reguł biznesowych} -- automatyczna walidacja XOR lekcji (albo uczeń albo grupa), zakresu ocen, konfliktów czasowych, kompetencji nauczycieli
...
\begin{enumerate}
    ...
    \item \textbf{Jeden przedmiot na nauczyciela:} każdy nauczyciel uczy jednego przedmiotu (REF)
    ...
    \item \textbf{Typy sal:} indywidualna (do 5 osób) lub grupowa (do 25 osób)
...
t\_lekcja & 2 & Lekcja z XOR: REF do ucznia lub grupy \\
...
Reprezentuje pojedynczą lekcję muzyki z mechanizmem XOR (albo uczeń albo grupa).
...
\begin{itemize}[nosep]
    ...
    \item ref\_uczen (REF t\_uczen) -- referencja do ucznia (XOR: dla lekcji indywidualnej)
    \item ref\_grupa (REF t\_grupa) -- referencja do grupy (XOR: dla lekcji grupowej)
...
\textbf{Reguła XOR:} Lekcja musi mieć wypełnione ALBO ref\_uczen (lekcja indywidualna) ALBO ref\_grupa (lekcja grupowa), nigdy oba jednocześnie i nigdy żadne. Walidowane przez trigger \texttt{trg\_lekcja\_xor}.
...
\begin{tabular}{|l|l|l|}
\hline
\textbf{Tabela} & \textbf{Typ bazowy} & \textbf{Referencje (REF) / Kolekcje} \\
\hline
...
Zaplanowane lekcje muzyki z mechanizmem XOR.
...
\begin{itemize}[nosep]
    ...
    \item Referencje XOR: ref\_uczen (dla indywidualnych), ref\_grupa (dla grupowych)
    \item Trigger: trg\_lekcja\_xor wymusza regułę XOR
\end{itemize}
...
\hline
trg\_lekcja\_xor & BEFORE INSERT/UPDATE & Wymuszenie XOR: lekcja musi mieć ALBO ucznia ALBO grupę (nigdy oba, nigdy żadne) \\
\hline
...
Trigger wymuszający regułę XOR dla lekcji:
...
\hline
-20001 & trg\_lekcja\_xor & Naruszenie XOR (lekcja musi mieć ucznia LUB grupę) \\
\hline
...
\begin{itemize}[nosep]
    \item \textbf{Trigger XOR:} Lekcja bez ucznia i bez grupy → błąd -20001
...
04\_triggery.sql & 2 wyzwalacze walidacyjne (XOR, zakres ocen) \\
...
\begin{tabular}{|l|l|p{6cm}|}
\hline
\textbf{Wymaganie} & \textbf{Status} & \textbf{Realizacja} \\
\hline
Typy obiektowe z metodami & \checkmark & 8 typów, 10 metod (wiek, staz\_lat, pelne\_nazwisko, czy\_grupowy, lista\_wyposazenia, godzina\_koniec, czy\_indywidualna, opis\_oceny) \\
\hline
Tabele obiektowe & \checkmark & 7 tabel obiektowych \\
\hline
REF i DEREF & \checkmark & 9 referencji między obiektami \\
\hline
VARRAY & \checkmark & t\_wyposazenie w tabeli sale + przeszukiwanie w sala\_ma\_instrument() \\
\hline
Pakiety PL/SQL & \checkmark & 5 pakietów, 25 podprogramów publicznych \\
\hline
Kursory & \checkmark & Jawny (OPEN/FETCH/CLOSE) w lista\_uczniow\_grupy(), niejawny (FOR) w pozostałych \\
\hline
Obsługa błędów & \checkmark & RAISE\_APPLICATION\_ERROR z 14 kodami błędów \\
\hline
Wyzwalacze & \checkmark & 2 triggery: XOR lekcji, zakres ocen \\
\hline
Walidacja konfliktów & \checkmark & Funkcja prywatna sprawdz\_kolizje() \\
\hline
Heurystyka & \checkmark & Funkcja znajdz\_alternatywe() (First Fit) \\
\hline
\end{tabular}
\caption{Spełnienie wymagań projektu}
\end{table}
